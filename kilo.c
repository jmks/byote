...
void editorDelChar() {
  ...
  // at beginning of file, so nothing to delete
  ...
  if (E.cx > 0) {
    ...
    // move row's text to previous row
...
void editorFindCallback(char *query, int key) {
  ...
  for (i = 0; i < E.numrows; i++) {
    ...
    // wrap-around the current line from beginning to end or vice versa
